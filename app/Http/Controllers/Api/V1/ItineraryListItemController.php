...
class ItineraryListItemController extends Controller
{
    ...
    public function reorderChecklistItems(Request $request, Itinerary $itinerary, ItineraryList $list, ItineraryListItem $item): JsonResponse
    {
        abort_if($itinerary->user_id !== $request->user()->id, 403);
     ...
     * Remove the specified item.
     */
    public function deleteChecklistItem(Request $request, Itinerary $itinerary, ItineraryList $list, ItineraryListItem $item, ItineraryListItemChecklistItem $checklistItem): JsonResponse
    {
        abort_if($itinerary->user_id !== $request->user()->id, 403);
        abort_if($item->itinerary_lists_id !== $list->id, 404);
    ...
    private function updateChecklist(ItineraryListItem $item, UpdateItineraryListItemRequest $request): void
    {
        if ($request->has('title')) {
            ...
            if ($request->has('checklist_item_id')) {
                $item->checklistItems()
                    ->where('id', $request->checklist_item_id)->update($request->safe()->except(['type', 'checklist_item_id']));
            } else {
                $item->checklistItems()->create([
                    ...$request->safe()->only(['label']),
                    'sort_order' => $item->checklistItems()->max('sort_order') + 1,
                ]);
            }

            $item->load('checklistItems');
        }
    }
